...
_C.TRAIN.LR_SCHEDULER.DECAY_EPOCHS = 0.3
...
def update_config(config, args):
    ...
    config.DATA.RATIO = args.ratio
    config.DATA.IMG_SIZE = args.img_size
    config.TAG = args.split
    ...
    # output folder
    config.OUTPUT = os.path.join(config.OUTPUT, config.DATA.DATASET+'_'+str(config.DATA.IMG_SIZE)+'_'+str(config.DATA.RATIO), str(config.TRAIN.BASE_LR)+'_'+str(config.TRAIN.WEIGHT_DECAY)+'_'+str(config.DATA.BATCH_SIZE*args.gpu_num), config.MODEL.TYPE, str(config.TAG))
